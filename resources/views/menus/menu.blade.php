              ...
              <div class="adv-table">
                <table cellpadding="0" cellspacing="0" border="0" class="display table table-bordered" id="hidden-table-info"  style="text-align: center;">
                    ...
                    <tr>
                        ...
                        <td>
                            @if($menu->tipe==0)
                              <span class="label label-primary label-mini"> Sarapan</span>
                            @elseif($menu->tipe==1)
                              <span class="label label-danger label-mini"> Makan Siang</span>
                            @elseif($menu->tipe==2)
                        ...
                        <td hidden="">
                                @foreach($menu_pembukas as $menu_pembuka)
                                    @if($menu_pembuka->id==$menu->menu_pembuka)
                                        {{$menu_pembuka->nama}}
                                    @endif
                                @endforeach
                        </td>
                        <td hidden="">
                                @foreach($menu_utamas as $menu_utama)
                                    @if($menu_utama->id==$menu->menu_utama)
                                        {{$menu_utama->nama}}
                                    @endif
                                @endforeach
                        </td>
                        <td hidden="">
                                @foreach($menu_penutups as $menu_penutup)
                                    @if($menu_penutup->id==$menu->menu_penutup)
                                        {{$menu_penutup->nama}}
                                    @endif
                                @endforeach
                        </td>
                        <td hidden="">
                                @foreach($minumans as $minuman)
                                    @if($minuman->id==$menu->minuman)
                                        {{$minuman->nama}}
                                    @endif
                                @endforeach
                                @foreach($minuman_bahan as $minumanbahan)
                                    @if($minumanbahan->id==$menu->minuman)
                                        {{$minumanbahan->nama}}
                                    @endif
                                @endforeach
                        </td>
              ...
              <div class="modal-body">
                ...
                <form action="#" class="form-horizontal" method="POST" >
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select class="form-control" name="menu_pembuka">
                            @foreach($menu_pembukas as $menu_pembuka)
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select class="form-control" name="menu_penutup">
                            @foreach($menu_penutups as $menu_penutup)
...
<script type="text/javascript">
  ...
  function fnFormatDetails ( oTable, nTr, id_bahan )
  {
    ...
    var sOut = '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">';
      ...
      sOut += '<tr><td>Menu Pembuka</td><td>=></td><td> '+aData[5]+'</td></tr>';
      sOut += '<tr><td>Menu Utama</td><td>=></td><td> '+aData[6]+'</td></tr>';
      sOut += '<tr><td>Menu Penutup</td><td>=></td><td> '+aData[7]+'</td></tr>';
      sOut += '<tr><td>Minuman</td><td>=></td><td> '+aData[8]+'</td></tr>';
      sOut += '</table>';
      return sOut;
  }
